...
export function registerPowerPlatformTools(server, service) {
    ...
    // Read-only tool registrations (39 tools)
    ...
    server.tool("query-records", "Query records using an OData filter expression. Use the 'select' parameter to limit returned columns and reduce response size.", {
        entityNamePlural: z.string().describe("The plural name of the entity (e.g., 'accounts', 'contacts')"),
    ...
    server.tool("get-relationship-details", "Get detailed metadata about a relationship", {
    ...
    server.tool("get-webresource-dependencies", "Get all dependencies for a web resource", {
    ...
    server.tool("preview-unpublished-changes", "Preview all components with unpublished customizations", {}, async () => {
        try {
            const service = getPowerPlatformService();
    ...
    server.tool("validate-solution-integrity", "Validate a solution's integrity and check for missing dependencies", {
    ...
    server.tool("check-entity-dependencies", "Check dependencies for a specific entity before deletion", {
        entityLogicalName: z.string().describe("Entity logical name")
    }, async ({ entityLogicalName }) => {
        try {
            const service = getPowerPlatformService();
    ...
    server.tool("get-entity-customization-info", "Get entity customization information (customizable, managed, custom)", {
        entityLogicalName: z.string().describe("Entity logical name")
    }, async ({ entityLogicalName }) => {
        try {
            const service = getPowerPlatformService();
    ...
    server.tool('find-documented-automations-by-table', 'Fast search for automations with YAML metadata indicating they modify a specific table', {
        tableName: z.string().describe('Table logical name (e.g., "contact")'),
        includeFlows: z.boolean().optional().describe('Include Power Automate flows (default: true)'),
        includeWorkflows: z.boolean().optional().describe('Include classic workflows (default: true)'),
        activeOnly: z.boolean().optional().describe('Only include active automations (default: true)')
    }, async (args) => {
        try {
            const service = getPowerPlatformService();
            const result = await service.findDocumentedAutomationsByTable(args.tableName, {
            ...
            console.error('Error finding documented automations:', error);
            return {
                content: [{
                        type: 'text',
                        text: `Failed to find documented automations: ${error.message}`
                    }],
                isError: true
            };
        }
    });
    server.tool('analyze-undocumented-automations', 'Deep analysis of automations lacking YAML metadata to find which modify a specific table', {
        tableName: z.string().describe('Table to search for modifications'),
        includeFlows: z.boolean().optional().describe('Include Power Automate flows (default: true)'),
        includeWorkflows: z.boolean().optional().describe('Include classic workflows (default: true)'),
        activeOnly: z.boolean().optional().describe('Only include active automations (default: true)'),
        maxToAnalyze: z.number().optional().describe('Maximum number of automations to analyze (default: 50)')
    }, async (args) => {
        try {
            const service = getPowerPlatformService();
            const result = await service.analyzeUndocumentedAutomations(args.tableName, {
                includeFlows: args.includeFlows,
                includeWorkflows: args.includeWorkflows,
                activeOnly: args.activeOnly,
                maxToAnalyze: args.maxToAnalyze
            });
            return {
                content: [{
                        type: 'text',
            ...
            console.error('Error analyzing undocumented automations:', error);
            return {
                content: [{
                        type: 'text',
                        text: `Failed to analyze undocumented automations: ${error.message}`
                    }],
                isError: true
            };
        }
    });
    console.error(`✅ PowerPlatform read-only tools registered (${41} tools, ${11} prompts)`);
}
// Export service class and config for use in other packages
