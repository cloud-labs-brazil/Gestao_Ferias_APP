 ...
 * These are Microsoft's out-of-the-box flows that should be excluded by default.
 * Last updated: v25.0.0
 */
...
export class PowerPlatformService {
    ...
    async getEntityAttributes(entityName, options) {
        ...
        const response = await this.makeRequest(`api/data/v9.2/EntityDefinitions(LogicalName='${entityName}')/Attributes?$select=${selectProperties}&$filter=${filterString}`);
        if (response && response.value) {
            ...
            if (options?.prefix) {
                ...
                response.value = response.value.filter((attr) => attr.LogicalName?.toLowerCase().startsWith(prefix));
            }
        }
        ...
        const formattedAttributes = (response.value || []).map((attr) => ({
    ...
    async getFlows(options = {}) {
        // Apply defaults - exclude non-custom flows by default (BREAKING CHANGE in v25.0.0)
        const { activeOnly = false, maxRecords = 25, excludeCustomerInsights = true, excludeSystem = true, excludeCopilotSales = true, nameContains, } = options;
        ...
        const filterConditions = ['category eq 5'];
        if (activeOnly) {
            filterConditions.push('statecode eq 1');
        }
        // Server-side filter: CXP_ prefix (Customer Insights)
        // OData 'not startswith' is supported in Dataverse Web API
        if (excludeCustomerInsights) {
            filterConditions.push("not startswith(name,'CXP_')");
        }
        // Server-side filter: nameContains (case-insensitive search)
        if (nameContains) {
        ...
        // Request more records to account for client-side filtering
        // If we need 25 and ~7% are SYSTEM/Copilot, request ~50% more to be safe
        ...
        const flows = await this.makeRequest(`api/data/v9.2/workflows?$filter=${filterString}&$select=workflowid,name,statecode,statuscode,description,createdon,modifiedon,type,ismanaged,iscrmuiworkflow,primaryentity,_ownerid_value&$expand=modifiedby($select=fullname)&$orderby=modifiedon desc&$top=${requestLimit}`);
        // Track exclusion statistics
        const excludedCounts = {
            customerInsights: 0, // Filtered server-side, can't count exactly
            system: 0,
            copilotSales: 0,
        };
        // Client-side filtering for SYSTEM and Copilot for Sales
        let filteredFlows = flows.value;
        if (excludeSystem) {
            const beforeCount = filteredFlows.length;
            filteredFlows = filteredFlows.filter((flow) => {
                const modifiedBy = flow.modifiedby?.fullname;
        ...
        if (excludeCopilotSales) {
            const beforeCount = filteredFlows.length;
            filteredFlows = filteredFlows.filter((flow) => {
                return !COPILOT_SALES_FLOW_NAMES.has(flow.name);
            });
            excludedCounts.copilotSales = beforeCount - filteredFlows.length;
        }
        // Check if there are more records and trim to requested max
        const hasMore = filteredFlows.length > maxRecords;
        const trimmedFlows = hasMore ? filteredFlows.slice(0, maxRecords) : filteredFlows;
        // Format the results for better readability
        const formattedFlows = trimmedFlows.map((flow) => ({
    ...
    async searchWorkflows(options = {}) {
        const { name, primaryEntity, description, category, statecode, includeDescription = true, maxResults = 50, } = options;
        // Build OData filter conditions
        ...
        const filterString = filterConditions.length > 0 ? filterConditions.join(' and ') : '';
        // Request one more than max to check if there are more records
        const requestLimit = maxResults + 1;
        // Build select clause based on includeDescription
        ...
        const workflows = hasMore ? response.value.slice(0, maxResults) : response.value;
        ...
        const formattedWorkflows = workflows.map((workflow) => ({
            ...
            category: workflow.category === 0 ? 'Classic Workflow' : workflow.category === 5 ? 'Power Automate Flow' : `Other (${workflow.category})`,
    ...
    async getFlowDefinition(flowId, summary = false) {
        const flow = await this.makeRequest(`api/data/v9.2/workflows(${flowId})?$select=workflowid,name,statecode,statuscode,description,createdon,modifiedon,type,category,ismanaged,iscrmuiworkflow,primaryentity,clientdata&$expand=modifiedby($select=fullname),createdby($select=fullname)`);
        ...
        try {
            ...
            if (flowDef.properties?.definition?.triggers) {
                ...
                if (triggerNames.length > 0) {
                    ...
                    // Extract trigger type and fields for YAML documentation
                    if (trigger.type === 'OpenApiConnectionWebhook' || trigger.type === 'OpenApiConnection') {
                        const entityName = trigger.inputs?.parameters?.entityName;
                        const operationId = trigger.inputs?.host?.operationId || '';
                        if (entityName) {
                            // Detect operation type from operationId
                            if (operationId.includes('Create') || operationId.includes('OnNew')) {
                                summary.triggerInfo = `${entityName} create`;
                            }
                            else if (operationId.includes('Update') || operationId.includes('OnModified') || operationId.includes('OnUpdated')) {
                                // Extract filter expression to show conditions
                                const filterExpression = trigger.inputs?.parameters?.filterExpression;
                                if (filterExpression && typeof filterExpression === 'string') {
                                    // Extract field names from OData filter
                                    const fieldMatches = filterExpression.matchAll(/([a-z_]+)\s*(?:eq|ne|gt|lt|ge|le)/gi);
                                    for (const match of fieldMatches) {
                                        summary.triggerFields.push(match[1]);
                                    }
                            ...
                            else if (operationId.includes('Delete') || operationId.includes('OnDeleted')) {
                                summary.triggerInfo = `${entityName} delete`;
                            }
                            else {
                                // Fallback: use entity name with operation ID
                                summary.triggerInfo = `${entityName} ${operationId}`;
                            }
                        }
                        else {
                            // No entity name - try to extract from other properties
                        ...
                        summary.triggerInfo = trigger.type || 'unknown';
                    }
                }
            }
            // Recursive helper to process actions (including nested ones in Scope, Condition, Apply_to_each)
            const processActions = (actions, path = '') => {
                for (const [actionName, action] of Object.entries(actions)) {
                    ...
                    if (actionType === 'scope' && action.runAfter && Object.values(action.runAfter).some((r) => r.includes('Failed'))) {
                    ...
                    if (action.type === 'OpenApiConnection' || action.type === 'ApiConnection') {
                        ...
                        // Extract tables modified from Dataverse write actions
                        const operationId = action.inputs?.host?.operationId;
                        if (operationId && ['CreateRecord', 'UpdateRecord', 'DeleteRecord', 'UpsertRecord', 'AssociateRecords'].includes(operationId)) {
                            const entityName = action.inputs?.parameters?.entityName;
                            if (entityName && typeof entityName === 'string') {
                                summary.tablesModified.add(entityName.toLowerCase());
                            }
                        }
                        // Detect Custom API calls (Dataverse actions that are NOT standard CRUD)
                        // Custom APIs have operationId but are not in the standard CRUD list
                        if (operationId && !['CreateRecord', 'UpdateRecord', 'DeleteRecord', 'UpsertRecord', 'AssociateRecords', 'GetRecord', 'ListRecords', 'GetItem', 'ListItems'].includes(operationId)) {
                            // Check if this is a Dataverse connector
                            const apiId = action.inputs?.host?.apiId || '';
                            const connectionName = action.inputs?.host?.connectionName || '';
                            const isDataverseConnector = apiId.includes('commondataservice') ||
                                connectionName.includes('commondataservice') ||
                                connectorId?.toLowerCase().includes('commondataservice');
                            if (isDataverseConnector) {
                                // This is likely a Custom API call
                                summary.customApisCalled.add(operationId);
                            }
                        }
                    }
                    // Extract tables modified from HTTP calls to Dataverse API
                    if (action.type === 'Http' && action.inputs?.uri) {
                        const uri = action.inputs.uri;
                        const method = action.inputs.method?.toUpperCase();
                        if (method && ['POST', 'PATCH', 'DELETE', 'PUT'].includes(method)) {
                            const dataverseApiMatch = uri.match(/\/api\/data\/v\d+\.\d+\/([a-z_]+)/i);
                            if (dataverseApiMatch) {
                                const entitySetName = dataverseApiMatch[1];
                                summary.tablesModified.add(entitySetName.toLowerCase());
                            }
                        }
                    }
                    ...
                    if (action.actions) {
                        // Scope, Apply_to_each, Do_until
                        processActions(action.actions, fullPath);
                    }
                    if (action.then) {
                        // If condition - then branch
                        processActions(action.then, `${fullPath}/then`);
                    }
                    if (action.else) {
                        // If condition - else branch
                        processActions(action.else, `${fullPath}/else`);
                    }
                    if (action.cases) {
                        // Switch statement cases
                        for (const [caseName, caseActions] of Object.entries(action.cases)) {
                            if (caseActions.actions) {
                                processActions(caseActions.actions, `${fullPath}/case:${caseName}`);
                            }
                        }
                    }
                    if (action.default) {
                        // Switch default case
                        processActions(action.default, `${fullPath}/default`);
                    }
                }
            };
            ...
            summary.tablesModified = Array.from(summary.tablesModified);
            summary.customApisCalled = Array.from(summary.customApisCalled);
    ...
    async getEnvironmentId() {
        ...
        try {
            ...
            const orgResponse = await this.makeRequest('api/data/v9.2/organizations?$select=organizationid');
    ...
    parseWorkflowXamlSummary(xaml) {
        ...
        try {
            ...
            // Extract tables modified from EntitySchemaName attributes
            const createEntityMatches = xaml.matchAll(/<CreateEntity[^>]+EntitySchemaName="([^"]+)"/g);
            for (const match of createEntityMatches) {
                summary.tablesModified.add(match[1].toLowerCase());
            }
            const updateEntityMatches = xaml.matchAll(/<UpdateEntity[^>]+EntitySchemaName="([^"]+)"/g);
            for (const match of updateEntityMatches) {
                summary.tablesModified.add(match[1].toLowerCase());
            }
            const assignEntityMatches = xaml.matchAll(/<AssignEntity[^>]+EntitySchemaName="([^"]+)"/g);
            for (const match of assignEntityMatches) {
                summary.tablesModified.add(match[1].toLowerCase());
            }
            const setStateEntityMatches = xaml.matchAll(/<SetState[^>]+EntitySchemaName="([^"]+)"/g);
            ...
            // Extract trigger information from XAML
            const triggerOnCreateMatch = xaml.match(/<TriggerOnCreate>([^<]+)<\/TriggerOnCreate>/);
            const triggerOnDeleteMatch = xaml.match(/<TriggerOnDelete>([^<]+)<\/TriggerOnDelete>/);
            const primaryEntityMatch = xaml.match(/<PrimaryEntity>([^<]+)<\/PrimaryEntity>/);
            if (primaryEntityMatch) {
                ...
                if (triggerOnCreateMatch && triggerOnCreateMatch[1] === 'true') {
                    ...
                    const triggerOnUpdateMatch = xaml.match(/<TriggerOnUpdate>([^<]+)<\/TriggerOnUpdate>/);
                    if (triggerOnUpdateMatch && triggerOnUpdateMatch[1] === 'true') {
                        summary.triggerInfo = `${primaryEntity}.update`;
                        // Extract filtering attributes (trigger fields)
                        const filteringAttributesMatch = xaml.match(/<FilteringAttributes>([^<]+)<\/FilteringAttributes>/);
                        if (filteringAttributesMatch) {
                            const attrs = filteringAttributesMatch[1].split(',').map(a => a.trim()).filter(a => a);
  ...
  <Area Id="Area_Tables" Title="Tables" ShowGroups="true">
     ...
     */
    async validateBestPractices(solutionUniqueName, entityLogicalNames, publisherPrefix, recentDays = 30, includeRefDataTables = true, rules = ['prefix', 'lowercase', 'lookup', 'optionset', 'required-column', 'entity-icon'], maxEntities = 0, requiredColumns = ['{prefix}updatedbyprocess']) {
        const timer = auditLogger.startTimer();
        ...
        try {
            ...
            if (solutionUniqueName) {
                // Get solution ID
                const solutionResponse = await this.makeRequest(`api/data/v9.2/solutions?$filter=uniquename eq '${solutionUniqueName}'&$select=solutionid,friendlyname,uniquename`);
                ...
                const componentsResponse = await this.makeRequest(`api/data/v9.2/solutioncomponents?$filter=_solutionid_value eq ${solutionId} and componenttype eq 1&$select=objectid`);
                ...
                for (const component of componentsResponse.value || []) {
                    ...
                    try {
                        ...
                        const entityResponse = await this.makeRequest(`api/data/v9.2/EntityDefinitions(${metadataId})?$select=LogicalName,SchemaName`);
            ...
            for (const entityLogicalName of entities) {
                try {
                    ...
                    const entityMetadata = await this.makeRequest(`api/data/v9.2/EntityDefinitions(LogicalName='${entityLogicalName}')?$select=LogicalName,SchemaName,DisplayName,MetadataId,IconVectorName,IsCustomEntity`);
                    // Get all attributes for entity
                    const attributesResponse = await this.makeRequest(`api/data/v9.2/EntityDefinitions(LogicalName='${entityLogicalName}')/Attributes?$select=LogicalName,AttributeType,DisplayName,CreatedOn,IsCustomAttribute,AttributeTypeName`);
            ...
            for (const attr of filteredAttributes) {
                ...
                if (isPicklist) {
                    try {
                        ...
                        const attrDetails = await this.makeRequest(`api/data/v9.2/EntityDefinitions(LogicalName='${entityMetadata.LogicalName}')/Attributes(LogicalName='${attr.LogicalName}')?$select=LogicalName&$expand=OptionSet($select=IsGlobal)`);
                        if (attrDetails.OptionSet && !attrDetails.OptionSet.IsGlobal) {
                            violations.push({
                                ...
                                severity: 'MUST',
                                message: `Option set "${attr.LogicalName}" is local, must be global`,
    ...
    buildViolationsSummary(entities) {
        ...
        for (const [rule, items] of grouped.entries()) {
            ...
            const affectedColumns = [...new Set(columnLevelViolations.map(v => `${v.entityLogicalName}.${v.attributeLogicalName}`))];
     ...
     */
    async findDocumentedAutomationsByTable(tableName, options = {}) {
        const { includeFlows = true, includeWorkflows = true, activeOnly = true } = options;
        ...
        if (includeFlows)
            categoryFilters.push('category eq 5');
        if (includeWorkflows)
            categoryFilters.push('category eq 0');
        if (categoryFilters.length > 0) {
            filters.push(`(${categoryFilters.join(' or ')})`);
        }
        // Filter by state (active only)
        if (activeOnly) {
            filters.push('statecode eq 1'); // 1 = activated
        }
        // Filter by documentation presence
        filters.push("contains(description, 'tables_modified:')");
        filters.push(`contains(description, '${tableName}')`);
        const filterString = filters.join(' and ');
        // Query documented automations
        const documentedResponse = await fetch(`${this.config.organizationUrl}/api/data/v9.2/workflows?$select=workflowid,name,category,description&$filter=${encodeURIComponent(filterString)}`, {
            headers: {
        ...
        if (!documentedResponse.ok) {
            throw new Error(`Failed to query documented automations: ${documentedResponse.status}`);
        }
        const documentedData = await documentedResponse.json();
        // Query all automations (to count undocumented)
        const allFilters = filters.filter(f => !f.includes('tables_modified') && !f.includes(tableName));
        const allFilterString = allFilters.join(' and ');
        const allResponse = await fetch(`${this.config.organizationUrl}/api/data/v9.2/workflows?$select=workflowid&$filter=${encodeURIComponent(allFilterString)}`, {
            headers: {
                'Authorization': `Bearer ${accessToken}`,
                'Accept': 'application/json',
                'OData-MaxVersion': '4.0',
        ...
        if (!allResponse.ok) {
            throw new Error(`Failed to query all automations: ${allResponse.status}`);
        }
        const allData = await allResponse.json();
        // Parse documented automations to extract metadata
        const matches = documentedData.value.map((workflow) => {
            // Extract tables_modified from YAML block
            const tablesModifiedMatch = workflow.description?.match(/tables_modified:\s*([^\n]+)/);
            const tablesModified = tablesModifiedMatch
                ? tablesModifiedMatch[1].split(',').map((t) => t.trim()).filter((t) => t && t !== 'none')
                : [];
            // Extract trigger
            const triggerMatch = workflow.description?.match(/trigger:\s*([^\n]+)/);
            const trigger = triggerMatch ? triggerMatch[1].trim() : 'unknown';
            return {
                id: workflow.workflowid,
                name: workflow.name,
                type: workflow.category === 5 ? 'flow' : 'workflow',
                tablesModified,
                trigger,
                description: workflow.description || ''
            };
        ...
        return {
            matches,
            undocumentedCount: allData.value.length - documentedData.value.length,
            searchedCount: allData.value.length
        };
    }
    /**
     ...
     */
    async analyzeUndocumentedAutomations(tableName, options = {}) {
        const { includeFlows = true, includeWorkflows = true, activeOnly = true, maxToAnalyze = 50 } = options;
        const accessToken = await this.getAccessToken();
        // Build filter for undocumented automations
        const filters = [];
        // Filter by type
        const categoryFilters = [];
        if (includeFlows)
        ...
        // Exclude documented automations
        filters.push("not contains(description, '[AUTO-DOCS:')");
        const filterString = filters.join(' and ');
        // Query undocumented automations
        const response = await fetch(`${this.config.organizationUrl}/api/data/v9.2/workflows?$select=workflowid,name,category,clientdata,xaml&$filter=${encodeURIComponent(filterString)}&$top=${maxToAnalyze}`, {
            headers: {
                'Authorization': `Bearer ${accessToken}`,
                'Accept': 'application/json',
                'OData-MaxVersion': '4.0',
        ...
        if (!response.ok) {
            throw new Error(`Failed to query undocumented automations: ${response.status}`);
        }
        const data = await response.json();
        const automations = data.value;
        // Parse each automation and filter by tableName
        const matches = [];
        let analyzedCount = 0;
        for (const automation of automations) {
            analyzedCount++;
            let summary;
            const isFlow = automation.category === 5;
            try {
                if (isFlow && automation.clientdata) {
                    const flowDefinition = JSON.parse(automation.clientdata);
                    summary = this.parseFlowSummary(flowDefinition);
                }
                else if (!isFlow && automation.xaml) {
                    summary = this.parseWorkflowXamlSummary(automation.xaml);
                }
                else {
                    continue; // Skip if no definition available
                }
                // Check if this automation modifies the target table
                if (summary.tablesModified.includes(tableName.toLowerCase())) {
                    matches.push({
                        id: automation.workflowid,
                        name: automation.name,
                        type: isFlow ? 'flow' : 'workflow',
                ...
                // Skip automations that fail to parse
                console.error(`Failed to parse automation ${automation.workflowid}:`, error);
            }
        }
        // Check if there are more automations beyond maxToAnalyze
        const countResponse = await fetch(`${this.config.organizationUrl}/api/data/v9.2/workflows?$filter=${encodeURIComponent(filterString)}&$count=true&$top=0`, {
            headers: {
                'Authorization': `Bearer ${accessToken}`,
                'Accept': 'application/json',
                'OData-MaxVersion': '4.0',
        ...
        let totalCount = analyzedCount;
        if (countResponse.ok) {
            const countData = await countResponse.json();
            totalCount = countData['@odata.count'] || analyzedCount;
        }
        return {
            matches,
            analyzedCount,
            skippedCount: Math.max(0, totalCount - analyzedCount)
        };
    }
}
